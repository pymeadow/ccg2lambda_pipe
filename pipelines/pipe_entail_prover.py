...
class EntailmentProver(TransformerMixin):
    """Adapt scripts/prove.py to scikit-learn transformer"""
    def __init__(self, prover_exe: str = "scripts/prove.py",
                 nltk_data_paths: str = [os.path.expanduser("~/nltk_data")],
                 coq_lib_path: str = "./coqlib.v",
                 output_dir: str = None,
                 do_abduction: str = "no",
        ...
        self.gold_trees = f"--gold_trees {gold_trees}"
        ...
        output_file = "{1}"
        output_html = "{2}"
        self.prove_command = (f"{self.prover_exe} {self.do_abduction} {self.gold_trees} " +
                              f"{print_output} {print_length} {self.timeout} {self.use_ncores} " +
                              f"--proof {output_file} --graph_out {output_html} {input_file}")
    ...
    def transform(self, input_file: str) -> str|None:
        ...
        run_command = shlex.split(f"{self.parse_command.format(input_file, output_file, output_html)}")
        try:
