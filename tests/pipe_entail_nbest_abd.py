...
def main():
    ...
    basic_pipe = Pipeline([
        ...
        ("sem_parser", CCGSemParser(nbest_output=3)),
        ("sem_writer", CCGTreeWriter(output_dir="./tmp", output_suffix="sem.xml")),
        ("sem_visual", CCGTreeVisualizer(output_dir="./tmp", output_suffix="sem")),
        ("entail_prover", COQEntailmentProver(do_abduction="spsa")), # no, naive, spsa
        ("proof_writer", CCGTreeWriter(output_dir="./tmp", output_suffix="pro.xml")),
        ("proof_visual", CCGTreeVisualizer(output_dir="./tmp", output_suffix="pro")),
        ("pivot", "passthrough")
        ])
